...
## 2024-06-11 - Unified/Remark Processor Instantiation Cost
**Learning:** Instantiating `unified().use(...)` processor pipelines on every execution creates a significant performance bottleneck due to the setup overhead.
**Action:** Always extract `unified` processor pipelines to module-level constants or class properties so they are initialized once and reused for `.parse()` and `.stringify()` operations.
