...
export async function compileMdx(source: string, scope: Record<string, unknown>): Promise<string> {
  ...
  // Create a vm context to evaluate expressions with better isolation than new Function
  // Note: node:vm does not provide a secure sandbox for untrusted code execution.
  ...
  visit(tree, (node, index, parent) => {
    if (node.type === 'mdxTextExpression' || node.type === 'mdxFlowExpression') {
      ...
      const result = String(vm.runInContext(`(function() { return ${expr}\n; })()`, context));
