...
export async function compileMdx(source: string, scope: Record<string, unknown>): Promise<string> {
  ...
  visit(tree, (node, index, parent) => {
    if (node.type === 'mdxTextExpression' || node.type === 'mdxFlowExpression') {
      ...
      // Use node:vm instead of new Function() to prevent arbitrary code execution vulnerabilities.
      const context = vm.createContext(Object.assign(Object.create(null), scope));
