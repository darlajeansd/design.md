...
export async function compileMdx(source: string, scope: Record<string, unknown>): Promise<string> {
  ...
  // Security note: node:vm provides basic isolation but is not a hardened secure sandbox.
  // It mitigates simple code injection but should not be relied upon for untrusted code execution.
  ...
  visit(tree, (node, index, parent) => {
    if (node.type === 'mdxTextExpression' || node.type === 'mdxFlowExpression') {
      ...
      const script = `(function() { return ${expr}\n; })()`;
      const result = String(vm.runInContext(script, context));
