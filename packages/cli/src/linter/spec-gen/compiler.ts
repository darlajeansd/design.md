...
// recreating them on every compileMdx call. This avoids redundant overhead and significantly
// improves compilation speed, especially when compiling many MDX files or sections.
const parserProcessor = unified()
  .use(remarkParse)
  .use(remarkMdx);

const stringifierProcessor = unified()
  .use(remarkStringify);
...
export async function compileMdx(source: string, scope: Record<string, unknown>): Promise<string> {
  ...
  const file = stringifierProcessor.stringify(tree);
